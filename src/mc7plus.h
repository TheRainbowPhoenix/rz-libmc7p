...
typedef enum {
        MC7P_FLOW_NONE = 0,
        MC7P_FLOW_LABEL,   /* label marker (label_id valid) */
        MC7P_FLOW_JMP,     /* unconditional jump to label_id */
        MC7P_FLOW_CJMP,    /* conditional jump ({COND} flag) to label_id */
        MC7P_FLOW_CALL,    /* block call (external, no in-blob target) */
        MC7P_FLOW_RET,     /* return */
} mc7p_flow_kind_t;

typedef struct {
        mc7p_flow_kind_t kind;
        long label_id;      /* LABEL/JMP/CJMP: label id, else -1 */
} mc7p_flow_t;

/* Decode one statement at code[off] and classify its control flow.
 * Plain JMP carries the target label in operand 0, every other JMP_*
 * variant in the LAST operand (the zymatik VM reads o[-1]);
 * the wire {COND} flag marks conditional jumps.  Returns the encoded
 * byte length, or -1 when nothing matches at off. */
